        ...
        <div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center py-4 sm:pt-0">
            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="testbox">
                    ...
                    <input id="check_in_date" type="date" name="check_in_date" />
                    <i class="fas fa-calendar-alt"></i>
                    </div>
                    <div class="item">
                    <label for="check_out_date">Check-out Date <span>*</span></label>
                    <input id="check_out_date" type="date" name="check_out_date" />
                    <i class="fas fa-calendar-alt"></i>
                    </div>
                    <div class="item">
                    <p>Check-in Time </p>
                    ...
                    <select name="check_out_time">
                        ...
                        @foreach(App\Models\ReservationForm::CHECK_OUT_TIME_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('check_out_time', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    </div>
                    <div class="item">
                    ...
                    <select name="num_of_adults">
                        <option value disabled {{ old('num_of_adults', null) === null ? 'selected' : '' }}>Number of adults</option>
                        @foreach(App\Models\ReservationForm::NUMBER_OF_ADULT_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('num_of_adults', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    </div>
                    <div class="item">
                    ...
                    <select  name="num_of_children">
                        <option value disabled {{ old('num_of_children', null) === null ? 'selected' : '' }}>Number of children</option>
                        @foreach(App\Models\ReservationForm::NUMBER_OF_CHILDREN_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('num_of_children', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    </div>
                    <div class="item" style=width:100%>
                    <label for="num_of_rooms">Number of rooms</label>
                    <input id="num_of_rooms" type="number" name="num_of_rooms" />
                    </div>
                    <div id="room_type_container" class="row w-100">
                        <div class="item">
                            <p>Room 1 type</p>
                            <select name="rooms[]">
